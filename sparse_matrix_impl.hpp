        ...
        for (size_t i = 0; i < rows; ++i) {
            for (size_t j = 0; j < cols; ++j) {
                ...
                if (it != m_data.end()) {
                    ...
                    std::cout << "0\t"; // or std::cout << ".\t"; to indicate an empty cell
                }
            }
            std::cout << std::endl;
        }
...
T& SparseMatrix<T, Order>::operator()(std::size_t row, std::size_t col) {
    ...
    if (compression) {
        ...
        throw std::runtime_error("SparseMatrix::operator(): Cannot insert new elements in compressed state");
    } else {
...
template<typename U>
std::vector<U> SparseMatrix<T, Order>::operator*(const std::vector<U>& vec) const {
    if (cols != vec.size()) {
        throw std::invalid_argument("SparseMatrix::operator*: Matrix columns and vector size mismatch");
    }

    std::vector<T> result(rows, T());  // Initialize result vector with zeros

    if (compression) {
        for (std::size_t i = 0; i < ptr.size() - 1; ++i) // For each row (col) of the matrix
        {
            std::size_t inner_start = ptr[i]; // Find the values in said row (col)
            std::size_t inner_end = ptr[i + 1];
            if constexpr (Order == StorageOrder::RowMajor)
            {
                for (std::size_t j = inner_start; j < inner_end; ++j) // Iterate over such elements perform matrix vector computation
                    result[i] += values[j] * vec[indices[j]];
            }
            else if constexpr (Order == StorageOrder::ColumnMajor)
            {
                for (std::size_t k = inner_start; k < inner_end; ++k) // Iterate through each non-zero element in the column
                    result[indices[k]] += values[k] * vec[i]; // Accumulate the result by multiplying the non-zero element with the corresponding element of the vector
            }
        }
    } else {
...
template <class T, StorageOrder Order>
SparseMatrix<T, Order> SparseMatrix<T, Order>::from_matrix_market(const std::string& filename) {
